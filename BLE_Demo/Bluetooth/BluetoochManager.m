...
#define CARDREAD_COMMAND_FOR_FIRSTTIME  @"0101000080"
#define CARDREAD_COMMAND_FOR_SECONDTIME @"0101008080"

static const NSUInteger STANTARD_CARDDATA_LENGTH = 512;
...
- (instancetype)init{
    if (self = [super init]) {
        ...
        _dataDic = [NSMutableDictionary dictionaryWithCapacity:2];
...
- (void)isConnectingPeripheralDevice:(PeripheralDevice *)device withState:(BT40LayerResultTypeDef)state{
    ...
    if(state == BT40LayerResult_Success){
        ...
        NSString* command = CARDREAD_COMMAND_FOR_FIRSTTIME;
...
- (void)bleCardHandler:(BleCardHandler *)cardHander didReceiveData:(NSData *)data state:(CardOperationState)state{
    ...
    NSLog(@"%@接收的数据长度：%ld",cardHander,_resultData.length);
    
    if(_resultData.length < STANTARD_CARDDATA_LENGTH && state == CardOperationState_ReadCorrect){
        BleCardHandler* cardHandler = [self cardHandlerForPeripheralDevice:
                                       [Bluetooth40Layer currentDisposedDevice]];
        if(!cardHandler) return;
        NSString* command = CARDREAD_COMMAND_FOR_SECONDTIME;
        [cardHandler cardRequestWithCommand:command];
        
    }else{
        NSLog(@"最终的结果数据是：%@",[[NSString alloc] initWithData:_resultData encoding:NSUTF8StringEncoding]);
        if ([self.delegate respondsToSelector:@selector(didReceiveDisposedData:fromDevice:)]) {
            [self.delegate didReceiveDisposedData:_resultData fromDevice:[Bluetooth40Layer currentDisposedDevice]];
        }
